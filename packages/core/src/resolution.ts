...
/** Every front size and every `sdfRes` is a multiple of this. */
 ...
 * is a ceiling, and rounding it up would raise it. A zero-texel front is not allocatable.
 */
export function sizeForDisplay(o: DisplaySizeRequest): number {
  ...
  const wanted = SIZE_QUANTUM * Math.ceil(Math.max(0, o.cssPx * o.dpr) / SIZE_QUANTUM)
 ...
 * old formula computed as 499 while leaving `exact` undefined entirely.
 */
export function sdfResFor(frontLongSide: number): number {
  const raw = SIZE_QUANTUM * Math.ceil(Math.max(0, frontLongSide) / 128)
