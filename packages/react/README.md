...
- **`useCrumple(options)`** owns one sprite's whole lifetime against the nearest `useScene()` (or
  ...
  under the same key through `view.swapTo` — reduced motion is consulted at the swap, not cached
  at mount. It returns a `Crumple`: a reactive snapshot (`state`, `pose`, `shown`, `requested`,
...
`CrumpleOptions`, `CrumpleSnapshot`, `CrumpleState` and `CrumpleFrameStyle` are exported too, for
