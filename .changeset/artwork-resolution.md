...
**paper** is fitted into (front = `sizeForDisplay(cssPx × dpr)`, artwork AT LEAST `1 / (1 + 2·overscan)`
of it — an equality for a portrait or square source, more for a landscape one, since the margin is
reserved against the artwork's height — drawn 1:1 under `fit: 'contain'`). `artworkCssPx` is the CSS long side the **artwork** holds on
...
512×512 surface (1 MB) to 1216×1216 (5.9 MB), and a stage that reaches the new ceiling costs 16 MB
(2048×2048) where it used to cost at most 1 MB. This is the STAGE's own owned surface, not the
front LRU: `budget` bounds resident **fronts** only (`packages/core/src/front-lru.ts`'s own "the
pools are whole-stage"), so it does not shrink to compensate — and each resident front is now
roughly 5.6× larger in bytes at the same `cssPx`/dpr, so the same `budget: 64MB` holds
