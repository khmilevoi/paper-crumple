...
supersession emitting `end` before `start`, and a `play()` issued from inside a supersession's `end`
refused rather than installed, so a handler that re-plays on every `end` cannot make the
superseding call loop forever. `Run<R = PlayResult>`, a thenable that is not a
